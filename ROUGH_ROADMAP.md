...
Authoritative status, execution waves and evidence gates are maintained in
[`TASK5.md`](TASK5.md). Semantic implementation details and diagrams are in
...
| **T5.2** | Fixed Evidence Safety Contract | 🟢 ✅ | Enforce tenant opt-in, separation of metadata/content, encryption, always-on secret scanning, audited privileged raw access and 30-day deletion. Task6 owns later configurability. | Task4 encryption, identity, policy and retention contracts. | Customer threat-model acceptance remains required. |
| **T5.3** | OpenCode Evidence Ingestion and Storage | 🟡 ◐ | Accept idempotent OpenCode prompt/response/reasoning/tool evidence, retain provider identities and store encrypted content separately from metadata. | T5.1–T5.2; Task4 delivery/encryption/retention. | Live OpenCode/GitAI collector verification remains required; other providers wait for Task13. |
...
| **T5.5** | Evidence Graph APIs | 🟢 ✅ | Support forward and reverse traversal with tenant-safe filters, pagination, evidence/confidence and availability while keeping GitAI entities authoritative. | T5.1–T5.3; Task11 may later publish selected contracts. | Live database/query review remains part of release acceptance. |
| **T5.6** | Visual Evidence Explorer | 🟢 ✅ | Provide a functional summary, supporting graph/timeline, code-provenance drill-down, semantic search and explicit privileged raw-content reveal. | T5.5; Task10 owns later design-system polish. | Customer usability evaluation remains required. |
| **T5.7** | Pain-Point and Quality Analytics | 🟢 ✅ | Explain failed/retried/slow tools, prompt loops, Task2 rework, abandoned sessions and weak outcomes using transparent evidence rather than employee scores. | T5.3, T5.5; Task2 metric invariants. | Analysts must validate interpretation on realistic data. |
| **T5.8** | Code-to-Intention Reverse Engineering | 🟢 ✅ | Navigate commit/file/line → GitAI range/checkpoint/trace/session → provider evidence/intention, exposing gaps without inventing causality. | T5.1, T5.3, T5.5; Git Notes/range attribution. | Manual provenance audits remain required for release. |
| **T5.9** | Semantic Intention Review | 🟡 ◐ | Provide tenant-isolated encrypted intention indexing, exact/semantic retrieval and repository/commit/file/session/tool/model filters. | T5.2–T5.5; Task4 deletion/retention. | Relevance, bias and false-similarity evaluation remains required before release. |
| **T5.10** | Continuous Verification | 🟡 ◐ | Continuously verify taxonomy, privacy, authorization, redaction, collector mapping, semantic behavior and Task2/Task4 regressions. Customer-grade bundles/certification remain Tasks12/14. | T5.1–T5.9; Task12 verification UX. | Live database, provider and customer/security acceptance remain outstanding. |
