...
| **T5.10** | Continuous Verification | Continuously verify taxonomy, privacy, collector mapping, graph behavior, semantic quality and Task2/Task4 regressions. | 🟡 ◐ | 🟡 ◐ | Final CI confirmation of the browser-discovered semantic fixes remains. | T5.1–T5.9 | **Wave 7 · 9** | The controlled customer walkthrough and 127 local API/security/metric tests pass. GitAI's complete cross-platform matrix also passes. E5-12 becomes complete only after the latest TrackAI CI run confirms the relevance/runtime changes. Customer certification stays in Tasks12/14. |
...
|---|---|---|---:|---:|---|---|---|
| **E5-1** | One session with zero, one and multiple commits | T5.1 | 🟢 ✅ | 🟢 ✅ | Controlled rollback corpus passed against fresh pgvector PostgreSQL and left zero residual rows. | `a954e08`; CI `35489454491` | None |
| **E5-2** | One commit with multiple sessions, traces and checkpoints | T5.1, T5.5 | 🟢 ✅ | 🟢 ✅ | Ephemeral pgvector CI proves one commit traverses to two distinct OpenCode/GitAI sessions, traces, checkpoints and intentions while preserving explicit relationship bases. | `a954e08`, `bc735b8`; CI `35492910176` | None |
...
| **E5-9** | Embeddings, hybrid search, hard negatives and tenant isolation | T5.9 | 🟢 ✅ | 🟢 ✅ | The pinned local BGE gate proves normalized 384-dimensional embeddings, cosine/lexical retrieval, RRF, exact priority and tenant isolation. Browser acceptance additionally proves concise-query normalization, a semantic-only cosine floor, visible match reasons, self-story exclusion and suppression of the design-token hard negative. Latest CI confirmation is tracked by E5-12. | `44de334`, `2a3e528`, `31df089`, `865909e`, `f65b1a4`, `e880fbe`, `c5de2ff`; CI `35495908579` | None |
...
| **E5-11** | Task2 metric and Task4 security regression | T5.10 | 🟢 ✅ | 🟢 ✅ | TrackAI CI run `35495908579` passes the 123-test API/security/metric suite, fresh migrations, all current Task5 live gates, API/web TypeScript and production web build. GitAI PR 1 passes all 44 applicable checks across macOS, Ubuntu and Windows, including full tests, lint, format, docs, CodeQL, agent E2E, OpenCode E2E, performance smoke and Git compatibility. | TrackAI `865909e`; GitAI `00262a33f`, `06128fcf3`, `cb1db62df`; GitAI runs `35503427698`, `35503427702`, `35503427704`, `35503427735` | None |
| **E5-12** | Complete product-manager acceptance walkthrough | T5.6–T5.10 | 🟢 ✅ | 🟡 ◐ | The controlled browser walkthrough passes for Leader, Developer, Security administrator and ordinary developer roles, including exact/missing attribution, insight drill-down, search, similar work, raw reveal/audit and UI denial. Final latest-branch CI confirmation remains. | `f269626`, `f65b1a4`, `e880fbe`, `c5de2ff` | CI confirmation |
...
| TrackAI implementation checkpoints | `2fcfc9b`, `44de334`, `2a3e528`, `11a4025`, `056c475`, `31df089`, `016b207`, `f269626`, `f65b1a4`, `e880fbe`, `c5de2ff` |
...
| Date | Scope | Status | Evidence / next gate |
|---|---|---:|---|
| 2026-09-20 | OpenCode/GitAI collection | 🟢 ✅ | GitAI `a8d93aa20` and `00262a33f` prove read-only SQLite extraction, pre-serialization redaction, stable replay bodies/IDs, unavailable reasoning and exact tenant/repository binding. TrackAI `3df7341`, CI run `35489833224`, proves managed-machine HTTP ingestion, database persistence, replay deduplication, second-pass redaction, raw-read audit and fail-closed consent/grant/credential denial. |
...
| Date | Scope | Status | Evidence / next gate |
|---|---|---:|---|
| 2026-09-19 | Automated release verification | 🟡 ◐ | TrackAI's automated baseline passed before live gates were added. The earlier GitAI local sandbox and Rust-version caveats are superseded by the supported GitHub Actions matrix recorded below. E5-12 remains, so Task5 is not yet complete under the completion rule. |
| 2026-09-20 | Controlled corpus and rollback-only live harness | 🟢 ✅ | TrackAI `a954e08`, CI run `35489454491`: fresh pgvector migrations, schema gate and `task5-password-recovery-v1` pass. The live transaction verifies many-to-many identity, traces/checkpoints, PR history, exact/missing attribution, encrypted content, consent, cross-tenant semantic references, zero residual rows and unchanged Task2 lifecycle counts. The complete workflow also passes 119 API/security/metric tests, API/web TypeScript and the production web build. |
| 2026-09-20 | Authenticated OpenCode ingestion and replay | 🟢 ✅ | TrackAI `3df7341`, CI run `35489833224`: managed authentication, accepted first delivery, deduplicated replay, pre-storage redaction, unavailable reasoning, raw-read audit and consent/grant/credential denial all pass against ephemeral pgvector PostgreSQL. API/security/metric tests, schema/corpus gates, API/web TypeScript and production build remain green. |
| 2026-09-20 | Evidence graph pagination and reverse provenance | 🟢 ✅ | TrackAI `d0a6db1` and `90de8f2`, CI run `35490329769`: the live gate found and fixed cross-page relationship loss and duplicate normalized/stored links. Paginated traversal is now lossless and non-duplicating across commit/session/trace/checkpoint/intention roots, tenant isolation passes, exact file/line attribution resolves to its trace, and missing attribution remains an explicit gap. The complete workflow passes 121 API/security/metric tests, API/web TypeScript and the production web build. |
| 2026-09-20 | Multi-session commit identity | 🟢 ✅ | TrackAI `bc735b8`, CI run `35492910176`: one commit traverses to two distinct sessions, traces, GitAI checkpoints and intentions without combining or redefining their identities. E5-2 and T5.1 are complete. |
| 2026-09-20 | Raw-evidence authorization and response safety | 🟢 ✅ | TrackAI `6eaf9fb`, CI run `35493134057`: administrators and security auditors can reveal authorized raw evidence, ordinary users receive `403`, successful responses are `no-store`, audits exist only for authorized reads, secrets remain redacted and captured logs contain no raw content. E5-3 is complete. |
| 2026-09-20 | Retention and derived-data deletion | 🟢 ✅ | TrackAI `07bdbef`, CI run `35493393579`: corrections inherit the source expiry; raw content, intention versions, vectors, lexical indexes, jobs, legacy semantic documents, summaries and intention links are deleted; safe event metadata remains marked expired and the purge is audited. E5-10 and T5.2 are complete. |
...
| 2026-09-20 | Controlled customer browser acceptance | 🟡 ◐ | TrackAI `f269626`, `f65b1a4`, `e880fbe` and `c5de2ff`: Leader, Developer and Security workflows pass against the disposable acceptance environment. Exact and missing file/line attribution, friction drill-down, semantic/exact search, similar-work outcome comparison, authorized raw reveal/audit and ordinary-user UI denial were observed. The walkthrough exposed and fixed retryable semantic reindexing, Intel macOS Python dependency compatibility, self-story leakage and a concise-query hard negative. The 127-test local suite and both TypeScript projects pass; latest-branch CI remains before E5-12/T5.10 can close. |
