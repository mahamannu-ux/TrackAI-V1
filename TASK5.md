...
| **T5.10** | Continuous Verification | Continuously verify taxonomy, privacy, collector mapping, graph behavior, semantic quality and Task2/Task4 regressions. | 🟡 ◐ | 🟡 ◐ | Live provider, product and security acceptance required. | T5.1–T5.9 | **Wave 7 · 9** | Fresh pgvector migrations, schema verification, rollback-only corpus, API/security/metric tests, API/web compilation and production web build pass in CI. Provider/browser/semantic gates and E5-12 remain. Customer certification stays in Tasks12/14. |
...
| **E5-11** | Task2 metric and Task4 security regression | T5.10 | 🟢 ✅ | 🟡 ◐ | TrackAI CI run `35489454491` passes the 119-test API/security/metric suite, fresh migrations, Task5 schema/live corpus gates, API/web TypeScript and production web build. GitAI focused Task5 tests pass 5/5. Its full local suite still has the previously recorded sandbox listener failures and untouched lint baseline. | TrackAI `a954e08`; GitAI `00262a33f` | Run GitAI full suite/lint in supported CI, then release review |
