...
| 2026-09-20 | Controlled corpus and rollback-only live harness | 🟡 ◐ | `task5-password-recovery-v1` maps one bounded synthetic customer story to E5-1–E5-12. Structural tests pass in the 117/117 API suite. The live harness now checks many-to-many identity, traces/checkpoints, PR history, exact/missing attribution, encrypted content, consent, cross-tenant semantic references, rollback cleanup and Task2 lifecycle counts; it is wired into pgvector CI. Local execution remains pending because this sandbox blocks PostgreSQL shared-memory initialization before migrations can run. |
