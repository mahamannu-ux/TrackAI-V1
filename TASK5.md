...
| **T5.8** | Code-to-Intention Reverse Engineering | Navigate commit/file/line → GitAI range/checkpoint/trace/session → provider evidence/intention without claiming time proximity as causality. | 🟢 ✅ | 🟡 ◐ | Browser-level developer provenance walkthrough required. | T5.1, T5.3, T5.5 | **Wave 4 · 7** | Live database-backed work stories now prove exact file/line → trace traversal and the honest missing-attribution case. The customer UI walkthrough remains under E5-6/E5-12. |
...
| **T5.10** | Continuous Verification | Continuously verify taxonomy, privacy, collector mapping, graph behavior, semantic quality and Task2/Task4 regressions. | 🟡 ◐ | 🟡 ◐ | Product and security browser acceptance required. | T5.1–T5.9 | **Wave 7 · 9** | TrackAI live gates and 123 API/security/metric tests pass. GitAI's complete macOS/Ubuntu/Windows test, lint, format, documentation, CodeQL, E2E, OpenCode E2E, performance-smoke and Git-compatibility matrix also passes. Browser acceptance and E5-12 remain. Customer certification stays in Tasks12/14. |
...
| **E5-8** | File/line → Git Note → checkpoint/trace/session → intention | T5.8 | 🟢 ✅ | 🟡 ◐ | Ephemeral pgvector CI proves the work-story API follows an exact GitAI range to its trace and reports a separate unattributed line as missing without inference. Browser-level developer traversal remains. | `11a4025`, `d0a6db1`, `90de8f2`; CI `35490329769` | UI provenance walkthrough |
| **E5-9** | Embeddings, hybrid search, hard negatives and tenant isolation | T5.9 | 🟢 ✅ | 🟢 ✅ | Ephemeral pgvector CI runs the checksum-verified, revision-pinned local BGE model against a two-tenant corpus. It proves normalized 384-dimensional embeddings, cosine and lexical retrieval, RRF with explainable match reasons, exact priority, semantic paraphrases, graded hard negatives and tenant isolation. Exact intention search remains available when the vector model is offline. | `44de334`, `2a3e528`, `11a4025`, `31df089`, `e45c57c`, `6cacaf7`, `1c381e4`, `d980c23`, `865909e`; CI `35495908579` | None |
