...
import { rebuildCommitLifecycle } from './service';
...
async function main() {
  ...
  const confirmed = args.get('confirm') === 'REBUILD_LIFECYCLE';
  ...
  const result = await rebuildCommitLifecycle(tenant.id, repositoryUrl, confirmed);
  console.log(JSON.stringify({
    dryRun: !confirmed,
    tenant: tenant.domain,
    ...result,
    ...(confirmed ? {} : { next: 'rerun with --confirm=REBUILD_LIFECYCLE after review' }),
  }, null, 2));
}
